...
services_start() {
  for SERVICE in ${STATIC_SERVICES} ; do
    case "${SERVICE}" in
    mojodb-luna)
      service_start mojodb-luna -c ${BASE}/db8/files/conf/maindb.conf /var/db /var/mediadb
      ;;
    *)
...
STATIC_SERVICES=" filecache activitymanager mojodb-luna"
...
export LD_LIBRARY_PATH=/usr/local/webos/usr/lib:${LIB_DIR}:${USR_LIB_DIR}:${LD_LIBRARY_PATH}
