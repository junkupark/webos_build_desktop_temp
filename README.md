...
-------------
  ...
  * Install the following components needed to build (and run) Open webOS on the desktop by typing the following:
        ...
        $ sudo apt-get install lemon libgmp-dev
...
Just tested script modified for build DB8 and SettingsService recently version. DB8 uses submissions/137 and you can check it in build-webos-desktop.sh script.

SettingsService is not enabled in this branch.

maybe you need to 'sudo apt-get install lemon libgmp-dev' additional task
./build-webos-desktop.sh
sudo ./install-webos-desktop.sh
