...
#export PKG_CONFIG_PATH=/usr/local/webos/usr/share/pkgconfig/:$LUNA_STAGING/lib/pkgconfig:$LUNA_STAGING/usr/share/pkgconfig
export PKG_CONFIG_PATH=/usr/local/webos/usr/share/pkgconfig:/home/webos/luna-desktop-binaries/staging/lib/pkgconfig:$LUNA_STAGING/lib/pkgconfig:$LUNA_STAGING/usr/share/pkgconfig
...
###################################
#  Fetch and build pmloglib-private
###################################
function build_pmloglib-private
{
    do_fetch openwebos/pmloglib $1 pmloglib submissions/
    set_source_dir $BASE/pmloglib

    mkdir -p build-private
    cd build-private
    $CMAKE .. -DNO_TESTS=True -DNO_UTILS=True -DCMAKE_INSTALL_PREFIX=${LUNA_STAGING} -DCMAKE_BUILD_TYPE=Release -DBUILD_PRIVATE=ON
    make $JOBS
    make install
}

##############################
#  Fetch and build pmlogdaemon
##############################
function build_pmlogdaemon
{
    # do_fetch openwebos/pmlogdaemon $1 pmlogdaemon submissions/
    set_source_dir $BASE/pmlogdaemon
...
function build_rdx
{
    do_fetch openwebos/rdx $1 rdx submissions/
    set_source_dir $BASE/rdx
...
function build_luna-service2
{
    ...
    export LDFLAGS="-Wl,-rpath $LUNA_STAGING/usr/lib"
    $CMAKE -D WEBOS_INSTALL_ROOT:PATH=${LUNA_STAGING} ..
    make $JOBS
    make install
...
function build_db8
{
    ...
    export LDFLAGS="-Wl,-rpath-link $LUNA_STAGING/lib -Wl,-rpath-link $LUNA_STAGING/usr/lib"
...
function build_configurator
{
    ...
    patch < $SCRIPT_DIR/0001-Configurator-Link-PMLOG.patch

    mkdir -p build
    cd build
    export LDFLAGS="-Wl,-rpath-link $LUNA_STAGING/usr/lib"
    $CMAKE -D WEBOS_INSTALL_ROOT:PATH=${LUNA_STAGING} -DCMAKE_INSTALL_PREFIX=${LUNA_STAGING} ..
    make $JOBS
    make install
    # NOTE: Make binary findable in /usr/lib/luna so ls2 can match the role file
...
function build_activitymanager
{
    ...
    patch < $SCRIPT_DIR/0002-ActivityManager-Link-PMLOG.patch

    mkdir -p build
    cd build
    export LDFLAGS="-Wl,-rpath-link,$LUNA_STAGING/usr/lib,-rpath-link,$LUNA_STAGING/lib"
    $CMAKE -D WEBOS_INSTALL_ROOT:PATH=${LUNA_STAGING} -DCMAKE_INSTALL_PREFIX=${LUNA_STAGING} ..
    make $JOBS
    make install
    # NOTE: Make binary findable in /usr/lib/luna so ls2 can match the role file
...
function build_filecache
{
    ...
    patch < $SCRIPT_DIR/0003-filecache-Link-PMLOG.patch
...
build cmake
build cmake-modules-webos 19

build cjson 35
build pbnjson 25
build pmloglib 48
build luna-service2 166
...
#rm -f ~/luna-desktop-binaries/settingsservice/*.stamp
#build settingsservice 41
